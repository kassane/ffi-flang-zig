...
pub fn build(b: *std.Build) void {
    ...
    const testFiles = &.{
        "tests/test-CFI_address.cc",
    };
    inline for (testFiles) |file| {
        const exe = buildTests(b, exeInfo{
            .name = file[6 .. file.len - 3],
            .files = &.{file},
            .target = target,
            .optimize = optimize,
            .lib = libfortran,
...
fn buildTests(b: *std.Build, options: exeInfo) *std.Build.Step.Compile {
